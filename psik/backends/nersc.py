...
async def readlines(f) -> List[str]:
    """ NERSC's file type isn't always consistently
        bytes or str...
    """
    ...
    for f in await machine.ls(str(remote_dir)):
        if await f.is_file():
            ...
            if f.name.startswith("/"):
                ...
                continue
            lname = rename.get(f.name, f.name)
            local = local_dir/lname
            if local.exists():
                st = local.stat()
                # st_mtime is float, f.date is str...
...
async def poll(job: Job) -> None:
    ...
    async with AsyncClient(key=keypath) as client:
        ...
        user_scratch = Path(f"/pscratch/sd/{user_name[0]}/{user_name}")
        psik_prefix = user_scratch / "psik"
